...
def create_property_struct_from_entry(prop: Union[str, dict],
                                      prop_mapping: dict,
                                      is_optional: bool=False) -> dict:
    if isinstance(prop, str):
        if prop not in prop_mapping.keys():
            raise PropertyNotFoundError(f"Simple property {prop} is not part of the objects properties. Add it or specify it as a dict.")
...
def get_additional_parameters(descr: dict, action_key: str):
    ...
    if action_key not in descr.keys():
        raise ActionNotFoundError(f"'{action_key}' is missing from your description file")
